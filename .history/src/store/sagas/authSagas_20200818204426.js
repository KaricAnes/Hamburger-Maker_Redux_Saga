...
export function* authSaga(action) {
  ...
  yield axios
    ...
    .then((response) => {
      ...
      yield put(authSuccess(response.data.idToken, response.data.localId)); //authR.js const authSuccess
      yield put(checkAuthTimout(response.data.expiresIn)); //automatic logout(), ima u sebi tajmer. Nakon sat vremena tajmer okida logout()
    })
    .catch((err) => {
      ...
      /*
       ...
       yield put(authFailed(err.response.data.error)); //Auth.js  {this.props.error.message}
      //debugger;
    });
}
